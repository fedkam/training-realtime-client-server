...
const Example = () => {
  ...
  useEffect(() => {
    const closeConnect: any = connectEventSource();
    return () => closeConnect.close();
  }, []);

  const connectEventSource = async () => {
    const eventSource = new EventSource(`http://localhost:3002/connect`);
  ...
  const sendMessage = async () => {
    await axios.post(`http://localhost:3002/send-message`, {
